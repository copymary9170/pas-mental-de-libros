...
def _calidad_score(item):
    return sum([bool(item.get("portada_path")), bool(item.get("sinopsis")), bool(item.get("autor")), bool(item.get("anio")), int(item.get("capitulo_total") or 0) > 0, bool(item.get("url_fuente") or item.get("link_original"))])


def _calidad_100(item):
    return int(round((_calidad_score(item) / 6) * 100))
...
def render_buscador_avanzado(obras, buscar_global, guardar_importado):
    ...
    if st.button("⭐ Guardar búsqueda", key="guardar_busqueda_fav") and query.strip():
        if query.strip() not in st.session_state["busquedas_favoritas"]:
            st.session_state["busquedas_favoritas"].append(query.strip()); st.success("Búsqueda guardada.")
        else: st.info("Ya estaba guardada.")

    if st.button("Buscar", key="buscador_btn") and query.strip():
        if fuente == "Buscar en todo": resultados, kind = _buscar_en_todo(query, buscar_global)
        else:
            resultados, kind = buscar_global(query, fuente); resultados = [_normalizar_item(r, fuente, kind) for r in resultados]
        st.session_state["external_results"] = resultados; st.session_state["external_kind"] = kind; st.session_state["external_source"] = fuente; st.session_state["external_query"] = query.strip()
            ...
            with cqa:
                if st.button("✅ Importar cola", key="importar_cola"):
                    ...
                    for queued in st.session_state["import_queue"]:
                        guardar_importado(queued, _opciones_tipo(queued.get("kind") or kind)[0], estado_import); total += 1
                    st.session_state["import_queue"] = []; st.success(f"Importados desde la cola: {total}")
            with cqb:
    ...
    for r in results:
        grupo = r.get("grupo_resultado") or r.get("fuente_importacion") or "Otros"; duplicados_tmp = _detectar_duplicados(r.get("titulo", ""), obras); key = _item_key(r)
    ...
    for i, item in enumerate(filtered):
        ...
        titulo_original = item.get("titulo", ""); duplicados = _detectar_duplicados(titulo_original, obras); item_kind = item.get("kind") or kind; key = _item_key(item); is_fav = key in st.session_state["result_favorites"]; calidad = _calidad_100(item)
        ...
        if duplicados: st.warning("Posibles duplicados ya guardados: " + ", ".join([d.get("titulo", "Sin título") for d in duplicados]))

        col_heart, col_fast, col_queue = st.columns([1, 1, 1])
        with col_heart:
            if st.button("❤️ Quitar fav" if is_fav else "🤍 Favorito", key=f"heart_{i}"):
                if is_fav: st.session_state["result_favorites"].remove(key); st.success("Quitado de favoritos.")
                else: st.session_state["result_favorites"].append(key); st.success("Marcado como favorito.")
        with col_fast:
            if st.button("⚡ Importar rápido", key=f"quick_import_{i}"): guardar_importado(item, _opciones_tipo(item_kind)[0], estado_import); st.success(f"Importado rápido: {titulo_original}")
        with col_queue:
            if st.button("➕ Añadir a cola", key=f"queue_import_{i}"):
                ...
                if not exists: st.session_state["import_queue"].append(item); st.success("Añadido a la cola.")
                else: st.info("Ya estaba en la cola.")

        with st.expander("✏️ Revisar / editar antes de importar"):
            st.markdown("#### 1. Datos principales")
            tipo_opts = _opciones_tipo(item_kind); col_a, col_b = st.columns(2)
            with col_a:
