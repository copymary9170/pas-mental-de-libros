...
try:
    from src.utils import buscar_libros_openlibrary, buscar_series_tvmaze, buscar_peliculas_itunes, buscar_peliculas_tmdb, buscar_series_tmdb, buscar_manga_jikan, buscar_webnovel_openlibrary, buscar_kdramas_tmdb
except ImportError:
...
def book_card(row):
    ...
    progress = pct(row)
    return f"""
    <div class="bookmory-card"><div class="bookmory-cover">{cover_html}</div>
      <div class="bookmory-title">{esc(row.get('titulo'))}</div><div class="bookmory-author">{esc(row.get('autor') or 'Autor no indicado')}</div>
      <div class="bookmory-meta"><span>{esc(row.get('estado_lectura'))}</span><span>{esc(row.get('clasificacion'))}/10</span></div>
      <div class="bookmory-progress"><div style="width:{progress}%"></div></div><div class="bookmory-small">Cap. {esc(row.get('capitulo_actual',0))}/{esc(row.get('capitulo_total',0))}</div>
    </div>"""
...
def tv_card(row):
    ...
    progress = pct(row)
    return f"""
    <div class="tv-card"><div class="tv-poster">{cover_html}</div><div class="tv-info">
      <div class="tv-title">{esc(row.get('titulo'))}</div><div class="tv-sub">T{esc(row.get('temporada_actual') or 1)} · E{esc(row.get('capitulo_actual') or 0)} de {esc(row.get('capitulo_total') or 0)}</div>
      ...
      <div class="tv-progress"><div style="width:{progress}%"></div></div><div class="tv-note">{esc(row.get('motivo_estado') or row.get('sinopsis') or 'Sin opinion todavia')}</div>
    </div></div>"""

def guardar_importado(item, tipo, estado):
    add_obra({"titulo": item.get("titulo", "Sin titulo"), "autor": item.get("autor", ""), "tipo": tipo, "clasificacion": 0, "estado_lectura": estado, "estado_publicacion": item.get("estado_publicacion", "No aplica"), "temporada_actual": 1, "temporada_total": int(item.get("temporada_total") or 1), "capitulo_actual": 0, "capitulo_total": int(item.get("capitulo_total") or 0), "sinopsis": item.get("sinopsis", ""), "etiquetas": item.get("etiquetas", "importado"), "link_original": "", "link_respaldo": "", "portada_path": item.get("portada_path", ""), "respaldo_path": "", "motivo_estado": f"Importado desde base externa. Año: {item.get('anio') or 'N/D'}", "favorito": 0, "fecha_inicio": str(date.today()), "fecha_fin": None})
...
tab_search, tab_books, tab_tv, tab_add, tab_chapters, tab_stats, tab_export = st.tabs(["🔎 Buscar e importar", "📚 Biblioteca", "📺 Series y pelis", "➕ Agregar manual", "📝 Capitulos", "📊 Stats", "⬇️ Exportar"])
...
with tab_search:
    ...
    if not TMDB_API_KEY and fuente in ["Peliculas", "Series / anime / TV", "Kdramas"]:
        st.info("Para mejores resultados de peliculas/series/kdramas agrega TMDB_API_KEY en Streamlit Secrets. Sin clave se usa una busqueda secundaria.")
    ...
    if buscar and query.strip():
        if fuente == "Libros":
        ...
        elif fuente == "Manga / manhwa / novelas ligeras":
            st.session_state["external_results"] = buscar_manga_jikan(query.strip())
            st.session_state["external_kind"] = "manga"
        elif fuente == "Webnovels":
            st.session_state["external_results"] = buscar_webnovel_openlibrary(query.strip())
            st.session_state["external_kind"] = "webnovel"
            ...
            st.session_state["external_results"] = resultados
            st.session_state["external_kind"] = "kdrama"
        else:
    ...
    if results:
        ...
        for i, item in enumerate(results):
            ...
            st.divider()
    elif buscar and query.strip(): st.warning("No encontre resultados. Puedes agregarlo manualmente en la pestaña Agregar manual.")
...
with tab_add:
    ...
    modo = st.radio("Tipo de registro", ["Libro / fanfic / manga / webnovel", "Serie / anime / kdrama / pelicula"], horizontal=True)
    with st.form("obra_form"):
        col1, col2 = st.columns(2)
        with col1:
            titulo = st.text_input("Titulo *"); autor = st.text_input("Autor / creador / estudio"); tipo = st.selectbox("Tipo", BOOK_TYPES if modo.startswith("Libro") else TV_TYPES); clasificacion = st.slider("Nota", 0.0, 10.0, 0.0, 0.5); estado = st.selectbox("Estado", ESTADOS); estado_pub = st.selectbox("Estado de publicacion", ESTADOS_PUBLICACION)
        with col2:
            temporada_actual = st.number_input("Temporada actual", min_value=1, value=1, step=1, disabled=modo.startswith("Libro")); temporada_total = st.number_input("Temporadas totales", min_value=1, value=1, step=1, disabled=modo.startswith("Libro")); capitulo_actual = st.number_input("Capitulo / episodio actual", min_value=0, step=1); capitulo_total = st.number_input("Capitulos / episodios totales", min_value=0, step=1); etiquetas = st.text_input("Etiquetas"); favorito = st.checkbox("Favorito")
